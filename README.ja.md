...
`v0.6.0` では、一般化した47都道府県の地域形状、読みやすい都道府県の正式名称、地図からの直接選択をコメ収穫量表示に追加しました。公式指標は e-Stat のままです。地域形状は別出典の Natural Earth Admin-1 5.1.1 オープンデータであり、日本政府の公式データではありません。精密表示用の inset は追加せず、Mobile再設計は引き続き延期しています。

Natural Earth Admin-1 は beta で、原則として de facto（実効支配）境界を採用した一般化データです。日本政府の領土・管轄に関する公式見解を示すものではなく、法令、測量、境界確定その他の正確な行政区域確認には使用できません。固定アーカイブ、SHA-256、加工内容、利用条件、制約は [`DATA-SOURCES.md`](DATA-SOURCES.md) と公開 `Sources/License` ページに記載しています。

`v0.5.0` では、固定された1つのコンテキストペイン、アクティブレイヤーの読み取りブロック、テーマ／レイヤーの単一操作導線、選択対象だけを扱う1つのインスペクタ、明示操作で開くシグナル／比較ビューを備えたデスクトップ地図ワークスペースへ再構成しました。

アクティブレイヤーの読み取りブロックには、対象範囲、期間、単位、地図表現、欠損値の扱い、現在の出典をまとめています。従来URLとの互換性を保ち、物流経路は固定デモデータと明示し、型付き・出典付き指標がない物流影響レイヤーは無効のままです。
